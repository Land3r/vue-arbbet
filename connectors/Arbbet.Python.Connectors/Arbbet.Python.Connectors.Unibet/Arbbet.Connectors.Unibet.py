    ...
    for market in marketTypes:
        for sub_market in market['subMarkets']:
            ...
            if (events != None):
                for event in events:
                    # Event 
                    logging.debug("Processing event %s:%s", event['competition_name'], event['event_name'])

                    # 
        
                    bet = BetService.getBetByPlatformAndPlatform_Id(session, platform.Id, event['marketId'])
                    if (bet == None):
